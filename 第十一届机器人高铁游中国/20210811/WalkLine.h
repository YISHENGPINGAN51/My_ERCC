...
void Front_QTI_TimeForWard(int time);     //前面中间两个QTI按时前进巡线
void Behind_QTI_TimeForWard(int time);    //后面中间两个QTI按时前进巡线
void Front_QTI_TimeBackWard(int time);    //前面中间两个QTI按时后退巡线
void Behind_QTI_TimeBackWard(int time);   //后面中间两个QTI按时后退巡线

void WalkLine_ForWard(int flag);          //前进巡线  模式0中短距使用  模式1长距使用（可补增一段按时巡线）
void WalkLine_BackWard(int flag);         //后退巡线  模式0中短距使用  模式1长距使用（可补增一段按时巡线）
